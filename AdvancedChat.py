...
"""
Advanced Unified Chat — Streamlit (E2B-compatible) + Stop Button + Profile Login

Adds:
- ⏹ Stop generating button (interrupts streaming and commits partial text)
- 🧹 Clear Vision/Data context (Content Drawer)
- 👤 Username/Password profiles:
    * On sign-in, APIs & inputs are pre-filled from your saved profile
    * "Save settings to my profile" button to persist keys and inputs
...
# Credential store (local JSON file)
CREDS_PATH = os.path.join(os.getcwd(), ".advancedchat_store.json")
PBKDF2_ROUNDS = 200_000

# ─────────────────────────────────────────────────────────────────────────────
# Credentials: local profile store (username/password → saved settings)
# ─────────────────────────────────────────────────────────────────────────────
...
def _apply_settings_from_profile(settings: dict):
    """Load saved settings into session_state."""
    st.session_state.update({
        # chat
        "base_url": settings.get("base_url", st.session_state.base_url),
        "api_key": settings.get("api_key", st.session_state.api_key),
        "model": settings.get("model", st.session_state.model),
        "temperature": settings.get("temperature", st.session_state.temperature),
        "top_p": settings.get("top_p", st.session_state.top_p),
        "max_tokens": settings.get("max_tokens", st.session_state.max_tokens),
        "system_prompt": settings.get("system_prompt", st.session_state.system_prompt),
        # vision/data API keys
        "vision_api_key": settings.get("vision_api_key", st.session_state.vision_api_key),
        "tg_key": settings.get("tg_key", st.session_state.tg_key),
        "e2b_key": settings.get("e2b_key", st.session_state.e2b_key),
        "phi_openai_key": settings.get("phi_openai_key", st.session_state.phi_openai_key),
        # together model
        "tg_model_human": settings.get("tg_model_human", st.session_state.tg_model_human),
        "tg_model_id": settings.get("tg_model_id", st.session_state.tg_model_id),
    })
    # refresh client if creds changed
    st.session_state.client = LLMClient(st.session_state.base_url, st.session_state.api_key)

def _collect_settings_for_profile() -> dict:
    return {
        ...
        "vision_api_key": st.session_state.vision_api_key,
        "tg_key": st.session_state.tg_key,
        "e2b_key": st.session_state.e2b_key,
        "phi_openai_key": st.session_state.phi_openai_key,
        "tg_model_human": st.session_state.tg_model_human,
        "tg_model_id": st.session_state.tg_model_id,
    }
...
class LLMClient:
    ...
    def stream_chat_events(
        ...
        """
        Return the raw streaming iterator (so caller can implement Stop).
        """
...
def init_session_state():
    defaults = {
        ...
        "context_data_enabled": True,
        "context_buffer": [],
        "data_context_snapshot": None,  # snapshot explicitly pushed to drawer
        # vision
        "vision_api_key": ENV_GOOGLE_API_KEY,
        "vision_preset": "General Vision",
        "vision_last": None,
...
# ─────────────────────────────────────────────────────────────────────────────
# Sidebar controls (with Save-to-profile)
# ─────────────────────────────────────────────────────────────────────────────
def sidebar_controls():
    with st.sidebar:
        st.header("🔌 Chat Connection")
        ...
        st.markdown("---")
        st.subheader("🔑 Optional API Keys")
        with st.expander("Edit API keys & models (pre-filled from your profile)"):
            vision_api_key = st.text_input("Google API Key (Gemini, agno - Vision)", type="password", value=st.session_state.vision_api_key)
            tg_key         = st.text_input("Together API Key (Visual agent)", type="password", value=st.session_state.tg_key)
            e2b_key        = st.text_input("E2B API Key (Sandbox)", type="password", value=st.session_state.e2b_key)
            phi_openai_key = st.text_input("OpenAI API Key (φ / data analyst)", type="password", value=st.session_state.phi_openai_key)
        ...
        if cols[2].button("💾 Save settings to my profile"):
            if not st.session_state.logged_in or not st.session_state.username:
                st.error("Please sign in first (top of app).")
            else:
                store = _load_store()
                users = store.setdefault("users", {})
                u = users.get(st.session_state.username)
                if not u:
                    st.error("User not found. Sign in or create a profile first.")
                else:
                    u["settings"] = _collect_settings_for_profile()
                    _save_store(store)
                    st.success("Saved current settings to your profile.", icon="💾")
...
def build_messages() -> List[Dict[str, str]]:
    ...
    # Prefer explicit data snapshot if present; else latest assistant answer
    if st.session_state.context_data_enabled:
        data_ctx = st.session_state.data_context_snapshot
        if not data_ctx:
            for m in reversed(st.session_state.data_chat_history):
                if m.get("role") == "assistant":
                    data_ctx = m.get("content")
                    break
        if data_ctx:
            context_lines.append(f"## Attached data context\n{data_ctx}")
...
def safe_numeric_coercion(df: "pd.DataFrame") -> "pd.DataFrame":
    """Conservative numeric coercion (no errors='ignore')."""
    if pd is None:
        return df
    for col in df.columns:
        if df[col].dtype == "object":
...
# robust sandbox opener compatible with old/new E2B APIs
@contextmanager
def open_sandbox(e2b_key: str):
    """
    Open an E2B sandbox that works across API versions.
...
def login_screen() -> bool:
    st.title("🔐 Sign in to AdvancedChat")
    with st.form("login_form", clear_on_submit=False):
        username = st.text_input("Username")
        password = st.text_input("Password", type="password")
        c1, c2, c3 = st.columns(3)
        sign_in = c1.form_submit_button("Sign in")
        register = c2.form_submit_button("Create / Update profile with current settings")
        sign_out = c3.form_submit_button("Sign out")

    if sign_out and st.session_state.logged_in:
    ...
    if sign_in:
        ...
        if not user:
            ...
            if _verify_password(password, pw.get("salt",""), pw.get("hash","")):
                st.session_state.logged_in = True
                st.session_state.username = username
                # load settings if any
                if "settings" in user:
                    _apply_settings_from_profile(user["settings"])
                st.success(f"Welcome, {username}! Settings loaded.")
                return True
            else:
    ...
    if register:
        if not username or not password:
            ...
            users[username] = {
                "password": salted,
                "settings": _collect_settings_for_profile(),  # save current UI settings
            }
            _save_store(store)
            st.session_state.logged_in = True
            st.session_state.username = username
            st.success("Profile created/updated and signed in.")
            return True

    st.info("Enter your **username** and **password**. On sign-in, your API keys and inputs are pre-filled from your profile. You can edit them in the sidebar and click **Save settings to my profile**.")
...
def main():
    ...
    st.title(APP_TITLE)
    st.caption("Chat with your LM Studio/OpenAI-compatible model. Add context from Vision and Data tools. Everything in one place.")
    ...
    # ─────────────────────────────────────────────────────────────────────────
    with tab_chat:
        # Commit any partial assistant text left from an interrupted stream
        if st.session_state.streaming_active and st.session_state.assistant_partial:
            st.info("⏹ Generation was stopped. Partial answer has been added to the conversation.")
            st.session_state.chat_history.append({"role": "assistant", "content": st.session_state.assistant_partial})
            st.session_state.assistant_partial = ""
            ...
            with colA:
                ...
                if st.session_state.vision_last:
                    ...
                    if st.button("➕ Inject this vision analysis into chat now"):
                        st.session_state.context_image_enabled = True
                        st.success("Vision analysis will be attached to the next chat turn.")
                    if st.button("🗑️ Clear Vision context"):
                        st.session_state.vision_last = None
                        st.session_state.vision_last_title = None
                        st.session_state.context_image_enabled = False
                        st.success("Cleared Vision context.")
                else:
                    st.caption("No vision analysis yet.")
            with colB:
                st.subheader("Data")
                # Prefer snapshot if set
                data_ctx = st.session_state.data_context_snapshot
                if not data_ctx:
                    for m in reversed(st.session_state.data_chat_history):
                        if m.get("role") == "assistant":
                            data_ctx = m["content"]; break
                if data_ctx:
                    st.markdown(data_ctx)
                    if st.button("➕ Inject this data answer into chat now"):
                        st.session_state.data_context_snapshot = data_ctx
                        st.session_state.context_data_enabled = True
                        st.success("Data-summary will be attached to the next chat turn.")
                    if st.button("🗑️ Clear Data context"):
                        st.session_state.data_context_snapshot = None
                        st.session_state.context_data_enabled = False
                        st.success("Cleared Data context.")
                else:
                    st.caption("No data-agent answer yet.")
            with colC:
                st.subheader("Quick actions")
                if st.button("🧽 Clear BOTH contexts"):
                    st.session_state.vision_last = None
                    st.session_state.vision_last_title = None
                    st.session_state.context_image_enabled = False
                    st.session_state.data_context_snapshot = None
                    st.session_state.context_data_enabled = False
                    st.success("Cleared Vision & Data contexts.")
        ...
        # Stop button shown at top of chat area
        stop_disabled = not st.session_state.streaming_active
        stop_col, _ = st.columns([1,4])
        if stop_col.button("⏹ Stop generating", disabled=stop_disabled):
        ...
        if user_input:
            ...
            # Stream reply with manual loop (to support Stop)
            with st.chat_message("assistant"):
                client: LLMClient = st.session_state.client
                messages = build_messages()
                ...
                st.session_state.assistant_partial = ""
                st.session_state.stop_chat = False  # reset before starting
                ...
                try:
                    ...
                    for event in stream:
                        ...
                        try:
                            ...
                            delta = ""
                        if delta:
                            st.session_state.assistant_partial += delta
                            placeholder.markdown(st.session_state.assistant_partial)
                except Exception as e:
                    st.session_state.assistant_partial += f"\n\n**[Error]** {e}"
                    placeholder.markdown(st.session_state.assistant_partial)

            # Finalize assistant message (either full or partial if stopped)
            st.session_state.chat_history.append({"role": "assistant", "content": st.session_state.assistant_partial})
            st.session_state.assistant_partial = ""
            st.session_state.streaming_active = False
            st.session_state.stop_chat = False
    ...
    with tab_data:
        ...
        if st.session_state.data_prepared:
            ...
            # Push explicit snapshot to Chat context
            if st.session_state.data_chat_history:
                if st.button("📎 Send latest data-agent answer to Chat"):
                    for m in reversed(st.session_state.data_chat_history):
                        if m.get("role") == "assistant":
                            st.session_state.data_context_snapshot = m["content"]
                            st.session_state.context_data_enabled = True
                            st.success("Latest data answer will be attached to the next chat turn.")
                            break
